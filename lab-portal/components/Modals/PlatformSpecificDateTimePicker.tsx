...
const PlatformSpecificDateTimePicker = ({ dateTime, onDateTimeChange, readOnly=false }: PlatformSpecificDateTimePickerProps) => {
  ...
  const formattedDate = moment(dateTime).format('MM/DD/YYYY hh:mm A'); // Format for display
  ...
  if (Platform.OS === 'web') {
    const localDateTime = moment(dateTime).local().format('YYYY-MM-DDTHH:mm'); // Ensure local time format
