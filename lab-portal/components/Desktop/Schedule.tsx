...
import { View, Text, StyleSheet, ActivityIndicator, ScrollView, Pressable, TouchableOpacity, TextInput } from 'react-native';
...
const LabSchedules = () => {
    ...
    if (selectedClearUserId !== null) {
      try {
        if (isDeptHead) {
          ...
          await ScheduleService.clearStudentSchedule(selectedClearUserId) // Clear student schedules 
        }
  ...
  const renderVerificationStatus = (scheduleId) => {
    ...
    if (exemption) {
      // Filter only the logged in user's verified schedules if they're not a department head
      if (exemption.verified || (!isDeptHead && user?.userId != exemption.fkUser)) {
  ...
  const handleScheduleClick = async (scheduleId, scheduleType) => {
    try {
      let scheduleInfo;
      if (scheduleType === 'Off' || (scheduleType == 'School' && isDeptHead)) {
        return;
      }
      console.log('Schedule Type:', scheduleType);

      if (["Work", "School"].includes(scheduleType)) {
        ...
        if (!isDeptHead && scheduleInfo.userId != user?.userId) {
          return;
        }
        ...
        if (!isDeptHead && scheduleInfo.fkUser != user?.userId) {
          return;
        }
        setSelectedUser(scheduleInfo.userId);
  ...
  const checkErrors = () => {
    ...
    if (formMode === "school") {
      if (location.length < 1)
      {
        ...
        error = error + 'Location must be between [1-50] characters\n';
      }
    }
    if (formMode != "school" && (!selectedLab || isNaN(selectedLab))) {
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      if (formMode === 'work' || formMode === 'school') {

        // For updating an existing schedule
        if (selectedSchedule) {
          check = {
            userID: isDeptHead ? selectedUser : user?.userId,
  ...
  const getFormComponents = () => {
    ...
    if (formMode === 'addOptions') {
      return [
        [
          <Pressable key="addSchedule" style={styles.optionButton} onPress={() => {
             setSelectedSchedule(null); 
             resetTimeValues();
             setFormMode(isDeptHead ? 'work' : 'school');
             }}>
            <Text style={styles.optionText}>{isDeptHead ? "Schedule Employee" : "Add School Schedule"}</Text>
          </Pressable>,
          <Pressable key="addExemption" style={styles.optionButton} onPress={handleAddExemption}>
            <Text style={styles.optionText}>Working Outside Of Schedule</Text>
          </Pressable>,
        ]
      ];
    }
    ...
    if (formMode === 'school') {
      components = [
        <View key="location" style={styles.formGroup}>
          <Text>
            <Text>Lab</Text>
            <TextInput
              value={location}
              onChangeText={setLocation}
              placeholder="Enter Class Name and Room Num"
    ...
    if (formMode.includes('exemption')) {
        ...
        components = [
          ...
          <View key="verified">
            ...
            <Checkbox 
              ...
              color={verified ? '#007BFF' : undefined} 
              disabled={!isDeptHead} 
            />
          </View>,
          <Pressable key="submitButton" style={styles.submitButton} onPress={handleFormSubmit}>
      ...
      if (formMode === 'exemption') {
        components = [
          ...
          <View key="lab" style={styles.formGroup}>
            <Text>
              <LabPicker selectedLabId={selectedLab} onLabChange={setSelectedLab} readOnly={exemptionType != 4} /> {/* Only allow users to change lab for working outside of schedule*/}
              {isLabError && <Text style={styles.errorAsterisk}>*</Text>}
            </Text>
          </View>,
          <View key="startDate">
    ...
    <View style={styles.container}>
            ...
            {Object.entries(scheduleData.reduce((acc, entry) => {
              ...
              <View key={userId} style={styles.tableRow}>
                <Text style={[styles.tableCell, styles.tableSubHeader]}>{[userEntries[0].user, newLine]}
                  {isDeptHead || (userId == user?.userId) && // Only allow deleting if they're a dept head or their own schedule
                    <>
                      <Pressable
      ...
      {isFormOpen && (
        <DynamicForm
          ...
          title={formMode.includes('exemption') ? 'Exemption' : (formMode === "school" ? 'School Schedule' : "Work Schedule")}
      ...
      <ConfirmationModal
        ...
        description={isDeptHead ? "Are you sure you want to clear this user's schedule?" : "Are you sure you want to clear your school schedule? This action cannot be undone."}
